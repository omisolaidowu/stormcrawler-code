...
``` sh
java -cp  target/storm-crawler-1.0.0.jar crawlercommons.urlfrontier.client.Client PutURLs -f seeds.txt
```
...
``` sh
storm local target/storm-crawler-1.0.0.jar --local-ttl 60 com.crawlme.CrawlTopology -- -conf crawler-conf.yaml
```
...
``` sh
storm local target/storm-crawler-1.0.0.jar  org.apache.storm.flux.Flux crawler.flux --local-ttl 3600
```
